...
import 'package:flutter/foundation.dart'
    show kIsWeb, Uint8List; // Para detectar plataforma web y manejar bytes.
...
// Soporta título, tiempo, ingredientes, instrucciones, foto y (NUEVO) categoría.
class AddEditRecipePage extends StatefulWidget {
  final String?
      recipeId; // ID opcional para modo edición (null para nueva receta).
...
class AddEditRecipePageState extends State<AddEditRecipePage> {
  ...
  // =========================
  //TODO: NUEVOS CAMPOS DE NOTAS , PROCIONES DE TIPO TEXTO CONTROLLER.

  //por defecto dificultad = Media, estacionalidad = Verano
  String _dificultad = 'Media'; // Fácil | Media | Difícil
  String _estacionalidad = 'Verano'; // Verano | Invierno
  ...
  void initState() {
    ...
    final recipe = widget.recipeId != null
        ? provider.getRecipeById(widget.recipeId!)
        : null;
    ...
    if (recipe != null &&
      ...
      _timeController = TextEditingController(
          text: (recipe.preparationTime ~/ 60).toString());
      _timeUnit = 'Horas';
    } else {
      _timeController =
    ...
    //TODO: INICIALIZAR NOTAS Y PORCIONES --> TextEditingController(text: '');
    ...
    if (recipe != null) {
      ...
      if (!kIsWeb &&
          ...
          File(recipe.photoPath!).existsSync()) {
      ...
      // _selectedCategory permanece en 'Comida' por defecto.
    }
  }
  ...
  void _addIngredientField() {
    setState(() {
      _ingredientControllers.add({
        'name':
            TextEditingController(), // Controlador para el nombre del ingrediente.
        'grams':
            TextEditingController(), // Controlador para la cantidad en gramos.
      });
    });
  }
  ...
  Future<void> _pickImage() async {
    try {
      ...
      if (pickedFile != null) {
        ...
        // Decodificar y redimensionar la imagen usando el paquete image.
  ...
  void _saveRecipe() {
    ...
    if (_formKey.currentState!.validate()) {
      ...
      if (ingredients.isEmpty) {
        _showErrorSnackBar(
            'Agrega al menos un ingrediente con nombre y cantidad');
        return;
      }
      ...
      // Si estamos editando, obtenemos la receta existente para preservar isFavorite.
      final existing = widget.recipeId != null
          ? provider.getRecipeById(widget.recipeId!)
          : null;

      // Crear la receta con TODOS los campos (incluida la categoría).
      final recipe = Recipe(
        ...
        instructions: _instructionsController.text.trim().isNotEmpty
            ...
            : null,
        photoPath:
            !kIsWeb ? _selectedImage?.path : null, // Usar path solo en nativo.
        photoBytes: _imageBytes, // Usar bytes para persistencia.
        isFavorite:
            existing?.isFavorite ?? false, //preserva favorito en edición
      );
            ...
            child: Form(
              ...
              child: Column(
                ...
                children: [
                  ...
                  //TODO: DIFICULTAD (FÁCIL, MEDIA, DIFÍCIL)
                  DropdownButtonFormField<String>(
                    initialValue: _dificultad,
                    items: const ['Fácil', 'Media', 'Difícil']
                  ...
                  //TODO: ESTACIONALIDAD (Verano/Invierno) --> HECHO

                  DropdownButtonFormField<String>(
                    initialValue: _estacionalidad,
                    items: const ['Verano', 'Invierno']
                        .map((e) => DropdownMenuItem(value: e, child: Text(e)))
                        .toList(),
                    onChanged: (v) =>
                        setState(() => _estacionalidad = v ?? 'Verano'),
                    decoration: InputDecoration(
                      labelText: 'Estacionalidad',
                      prefixIcon:
  ...
  Widget _buildPlaceholder() {
    return Container(
      ...
      child: Icon(Icons.image,
          size: 40, color: Colors.grey[600]), // Icono más pequeño.
    );
  }
